...
public final class JFileRenderer extends JRenderer<JFileData> {
    ...
    private final JInterfaceRenderer interfaceRenderer = new JInterfaceRenderer();
    private final JEnumerationRenderer enumerationRenderer = new JEnumerationRenderer();
    ...
    protected String doRender(JFileData data) {
        ...
        return packageRenderer.renderToString(data.getPackageData()) + System.lineSeparator()
                ...
                .map(interfaceRenderer::renderToString)
                .collect(Collectors.joining())
                //Enumerations
                + data.getTypes().stream()
                .filter(x -> x instanceof JEnumerationData)
                .map(x -> (JEnumerationData) x)
                .map(enumerationRenderer::renderToString)
                .collect(Collectors.joining());
    }
}
